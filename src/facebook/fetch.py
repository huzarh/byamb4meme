...
class FacebookFetch:
    def __init__(self) -> None:
        ...
        with sync_playwright() as p:
            self.browser = p.chromium.launch(headless=not DEBUG, channel="chrome", args=["--start-maximized"])
    ...
    def fetch_group_post_image(self, id: str, post: ElementHandle, source: ISource, reaction_count: int) -> None:
        ...
        if post_el is None:
            return
        post_el.click()
        fu_size = self.page.wait_for_selector("//a[contains(text(), 'full size')]")
        if fu_size is None:
            return
        fu_size.click()
    ...
    def get_articles(self) -> List[ElementHandle]:
        return self.page.query_selector_all("//div[@id='m_group_stories_container']//article")
