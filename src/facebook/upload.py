...
class FacebookUpload:
    def __init__(self) -> None:
        ...
        with sync_playwright() as p:
            ...
            self.page.wait_for_selector("//div[@aria-label='Switch Now']").click()  # type: ignore
    ...
    def post_contents(self) -> None:
        ...
        for post_id in Utils.get_list_static_files(STATIC_ROOT):
            try:
                ...
                if post_type == "photo":
                    self.page.goto("https://www.facebook.com/byamba4meme")
                    self.page.wait_for_selector("//span[contains(text(), 'Photo/video')]").click()  # type: ignore
                    ...
                    self.page.wait_for_load_state("networkidle")
                    self.page.wait_for_selector("//div[@role='dialog']//div[@aria-label='Post']").click()  # type:ignore
                    self.page.wait_for_load_state("networkidle")
                ...
                if post_type == "video":
                    ...
                    self.page.wait_for_load_state("networkidle")
                    self.page.wait_for_selector("//div[@aria-label='Next'][@role='button'][@tabindex='0']").click()  # type: ignore
                    self.page.wait_for_selector("//div[@data-lexical-editor='true'][@aria-label='Describe your reel...']").fill(Utils.get_upload_text(post_id))  # type:ignore
                    self.page.wait_for_selector("//div[@aria-label='Publish'][@tabindex='0']").click()  # type: ignore
                    self.page.wait_for_load_state("networkidle")
