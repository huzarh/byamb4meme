        ...
        for post_id in Utils.get_list_static_files(STATIC_ROOT):
            try:
                ...
                if not path.exists(file_path):
                    continue
                await self.send_message_with_attachment(Utils.get_upload_text(post_id), file_path)
                Utils.save_uploaded_content("discord", post_id)
            except Exception as e:
                print("\t[-] Error@discord_upload: {e}")

    async def send_message_with_attachment(self, message, file_path, add_separator=True) -> None:
        ...
        async def on_ready() -> None:
            with open(file_path, "rb") as f:
                file = discord.File(f)
                await client.get_channel(DISCORD_CHANNEL).send(content=message, file=file)  # type: ignore
                if add_separator:
                    await client.get_channel(DISCORD_CHANNEL).send(content="-" * 50)  # type: ignore
            await client.close()
