    ...
    def __init__(self) -> None:
        ...
        with sync_playwright() as p:
            self.browser = p.chromium.launch(
                headless=not DEBUG,
                channel="chrome",
                args=["--enable-popup-blocking", "--mute-audio", "--start-maximized"],
            )
    ...
    def post_contents(self) -> None:
        ...
        for post_id in Utils.get_list_static_files(STATIC_ROOT):
            ...
            try:
                ...
                self.page.wait_for_selector("//button[contains(text(), 'Not Now' )]", timeout=2_000).click()  # type:ignore
            except:
                pass
            ...
            try:
                ...
                if post_type == "video":
                    self.page.wait_for_selector("//button[contains(text(), 'OK' )]", timeout=3_000).click()  # type: ignore
            except:
                pass
            self.page.wait_for_timeout(2_000)
            # set view size to original
            self.page.query_selector_all("//div[@role='dialog']//button[@type='button']")[1].click()
            self.page.wait_for_selector("//div[contains(text(), 'Original')]").click()  # type: ignore

            # click next button
            self.page.wait_for_selector("//div[@role='dialog']//div[contains(text(), 'Next')]").click()  # type: ignore

            # click next button
            self.page.wait_for_selector("//div[@role='dialog']//div[contains(text(), 'Next')]").click()  # type: ignore

            # set description
            self.page.wait_for_selector("//div[@role='dialog']//div[@data-lexical-editor='true']").fill(Utils.get_upload_text(post_id))  # type: ignore

            # click share button
            self.page.wait_for_selector("//div[@role='dialog']//div[contains(text(), 'Share')]").click()  # type: ignore
